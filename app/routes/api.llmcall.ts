...
async function llmCallAction({ context, request }: ActionFunctionArgs) {
  ...
  if (streamOutput) {
    ...
    try {
      ...
      // Filter models by the requested provider so a model from a different provider doesn't cause a mismatch
      const providerModels = models.filter((m: ModelInfo) => m.provider === providerName);
      let modelDetails = providerModels.find((m: ModelInfo) => m.name === model);

      if (!modelDetails) {
        if (!providerModels.length) {
          throw new Error(`No models found for provider ${providerName}`);
        }
